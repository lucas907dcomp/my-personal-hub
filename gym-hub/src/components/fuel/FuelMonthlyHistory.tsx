  ...
  return (
    <section className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between p-5 text-left hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors focus:outline-none"
      >
        <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 uppercase tracking-widest">
          Histórico Mensal
        </span>
        <span className="text-slate-500 dark:text-slate-400 text-lg">{isOpen ? '▲' : '▼'}</span>
      </button>

      {isOpen && (
        <ul className="border-t border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-700/50">
          {history.map(entry => (
            <li key={entry.month} className="flex items-center justify-between px-5 py-3 text-sm">
              <span className="text-slate-600 dark:text-slate-300 font-medium">{formatMonth(entry.month)}</span>
